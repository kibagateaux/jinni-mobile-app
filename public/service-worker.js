...
const CacheStorageLoader = nunjucks.Loader.extend({
    ...
    getSource: async function (name, callback) {
        try {
            ...
            const cachedResponse = await caches.match(workbox.precaching.getCacheKeyForURL(path), {
...
const nunjucksEnv = new nunjucks.Environment(new CacheStorageLoader());
...
workbox.routing.registerRoute(
    new RegExp(
        'https://storage\\.googleapis\\.com/workbox-cdn/releases/.+/workbox-window\\.prod\\.mjs',
    ),
    new workbox.strategies.CacheFirst(),
);
